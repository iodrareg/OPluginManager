...
@PluginDescriptor(
    ...
    description = "Allows you to use prayers on the same tick as an action",
    tags = {"prayer", "one tick", "action"},
    enabledByDefault = false
)
@Slf4j
public class OneTickPrayerPlugin extends Plugin {
    ...
    @Inject
    private OneTickPrayerConfig config;
    @Inject
    PluginManager pluginManager;
    private final int quickPrayerWidgetID = WidgetInfo.MINIMAP_QUICK_PRAYER_ORB.getPackedId();

    @Provides
    public OneTickPrayerConfig getConfig(ConfigManager configManager) {
        return configManager.getConfig(OneTickPrayerConfig.class);
    }
